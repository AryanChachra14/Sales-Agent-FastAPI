import os, json, re, asyncpg, httpx, difflib
...
STATEMENT_TIMEOUT_MS = int(os.getenv("STATEMENT_TIMEOUT_MS", "2500"))
...
async def answer(body: AnswerReq):
    pool = app.state.pool
    q_in = (body.chatInput or "").strip()
    qn = normalize(q_in)

    if qn in SQL_CACHE:
        sql = SQL_CACHE[qn]
    else:
        if re.match(r"(?is)^\s*(select|with)\b", q_in):
            sql = sanitize_llm_sql(q_in)
        else:
            if not body.allow_llm:
                raise HTTPException(400, "No deterministic match and LLM disabled")
            sql = await gpt_sql_with_repair(q_in)
    ...
    try:
        ...
        sql = await repair_from_schema_mismatch(q_in, sql, e.detail)
        sql = scrub_illegal_numeric_casts(sql)
        sql = simplify_stage_literal(sql)
        validate_fields_against_schema(sql)
    ...
    if (body.needs_data or "YES").upper() == "NO":
        SQL_CACHE[qn] = sql
        return {"session_id": body.sessionId or "none", "needs_data": "NO", "question": q_in, "sql": sql, "data": []}
    ...
    try:
        async with pool.acquire() as conn:
            try:
                ...
                sql = await repair_from_error(q_in, sql, str(ve))
            await conn.execute(f"SET LOCAL statement_timeout = {STATEMENT_TIMEOUT_MS}")
            rows = await conn.fetch(sql)
        data = [dict(r) for r in rows]
        SQL_CACHE[qn] = sql
        return {"session_id": body.sessionId or "none", "needs_data": (body.needs_data or "YES").upper(), "question": q_in, "sql": sql, "data": data}
